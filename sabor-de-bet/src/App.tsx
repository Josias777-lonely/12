...
// Real-time server-side database integration (Firestore)
...
import { 
  ...
  TrendingUp, Users, Phone, Mail, Lock, Eye, EyeOff
} from 'lucide-react';
...
export default function App() {
  ...
  // --- Real-time Real Database Firestore synchronization ---
  useEffect(() => {
    const usersCollectionRef = collection(db, 'users');
    const unsubscribe = onSnapshot(usersCollectionRef, (snapshot) => {
      const dbUsers: any[] = [];
      snapshot.forEach(docSnap => {
        dbUsers.push(docSnap.data());
      });
      
      if (dbUsers.length === 0) {
        // Seed database if empty with default profiles
        const initialSeed = [
        ...
        initialSeed.forEach(async (usr) => {
          try {
            await setDoc(doc(db, 'users', usr.username.toLowerCase()), {
              ...usr,
  ...
  // Keep balance state and Firestore in perfect alignment
  useEffect(() => {
    localStorage.setItem('betfree_balance', balance.toString());
    if (isLoggedIn && loggedInUser) {
      const meRef = doc(db, 'users', loggedInUser.toLowerCase());
      updateDoc(meRef, {
        balance: balance,
        updatedAt: new Date().toISOString()
      }).catch(err => {
        // Safe lock - user document might be loading or provisioning, ignore temporary errors
        console.warn("Firestore balance update ignored during transient states", err);
      });
    }
  ...
  const handleClaimDailyReward = () => {
    if (!isLoggedIn) {
      alert("⚠️ Você precisa de uma conta ativa para resgatar coins! Faça login para ganhar +100 Coins.");
      setAuthMode('login');
  ...
  const handleWatchAd = () => {
    if (!isLoggedIn) {
      alert("⚠️ Você precisa de uma conta ativa para assistir anúncios! Faça login para ganhar +100 Coins.");
      setAuthMode('login');
      setIsAuthModalOpen(true);
      return;
    }
  ...
  const updateBalanceHandler = (amount: number) => {
    if (!isLoggedIn && amount !== 0) {
      alert("⚠️ Faça login para receber +100 Coins grátis e começar a simular seus palpites esportivos!");
      setAuthMode('login');
      setIsAuthModalOpen(true);
      return;
    }
  ...
  const handlePlaceSportsBetCoupon = (stake: number, totalOdd: number) => {
    if (!isLoggedIn) {
      alert("⚠️ Você precisa de uma conta ativa para apostar! Faça login para receber +100 Coins grátis!");
      setAuthMode('login');
      setIsAuthModalOpen(true);
      return;
    }
  ...
  const handleAuthSubmit = (e: React.FormEvent) => {
    ...
    if (authMode === 'login') {
      ...
      // Restore their real-time DB balance and give them +100 login award
      const userCoins = matchedUser.balance !== undefined ? matchedUser.balance : 100;
      const finalBalance = userCoins + 100;
      setBalance(finalBalance);

      // Save new balance to firestore immediately
      const meRef = doc(db, 'users', matchedUser.username.toLowerCase());
      updateDoc(meRef, {
        balance: finalBalance,
        updatedAt: new Date().toISOString()
      }).catch(err => {
        console.error("Erro ao sincronizar moedas após login: ", err);
      });
      ...
      alert(`Boas-vindas de volta, ${matchedUser.username}! 🎉 Seus coins foram sincronizados em tempo real com o banco de dados e você ganhou +100 Coins de bônus!`);
    } else {
      // REGISTER MODE
      // 1. Validate Username
      ...
      // Create document in real firestore database synchronously!
      setDoc(doc(db, 'users', lowercasedUsername), {
        ...newUser,
        updatedAt: new Date().toISOString()
      })
        .then(() => {
        ...
        .catch(err => {
          console.error("Cadastro falhou no Firestore: ", err);
  ...
  const handleSendSupportMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (cleanLower.includes('saldo') || cleanLower.includes('dinheiro') || cleanLower.includes('sacar') || cleanLower.includes('trocar')) {
        ...
        reply = '🎁 Dica: Você ganha +100 Coins adicionais ao fazer login, e também pode coletar o Bônus Diário de +300 Coins a cada 1 minuto clicando no botão do cabeçalho!';
      } else if (cleanLower.includes('ajuda') || cleanLower.includes('mines') || cleanLower.includes('perdi')) {
        reply = '🍀 Não desanime! Se suas fichas ficarem zeradas, faça login novamente para ativar os bônus, ou utilize o bônus diário para recuperar seu saldo de Coins!';
  ...
  const handleRedeemGiftCard = (service: string, cost: number) => {
    if (!isLoggedIn) {
      alert("⚠️ Você precisa de uma conta ativa para resgatar Gift Cards! Crie sua conta para ganhar +100 Coins.");
      setAuthMode('register');
      setIsAuthModalOpen(true);
      return;
    }
            ...
            <div className="bg-transparent border-none p-0 space-y-2 relative" id="search-nav-container">
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="py-2.5 px-4 bg-emerald-500 hover:bg-emerald-400 text-slate-950 hover:text-slate-900 font-sans font-bold text-xs rounded-xl transition cursor-pointer shrink-0 flex items-center gap-1.5 select-none"
                >
          ...
          >
            ...
            <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
              ...
              <div className="bg-slate-950/40 border border-slate-850 p-3 rounded-xl space-y-1">
                ...
                <p className="text-[11px] text-slate-400 leading-relaxed font-sans">
                  Com certeza! Basta tocar no botão "Bônus +300 FBF" no menu superior. Ele recarrega após 1 minuto para dar dinamismo aos testes.
                </p>
              </div>
            </div>
